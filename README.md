...
2. Equal contending powers are stable if sufficient deterrence mechanism is at place: second-strike capability, even if merely estimated by a node on other nodes, effectively nullifies the incentives to use check-in bots given total uniformity of competitive advantage.
3. Total uniformity of competitive advantage can only be maximal: To make point 2 possible, competitive advantage should be uniformly distributed. The only way where this is possible is if everybody is using the same allele variant of the tech (centralization), but the incentives to use the same allele variant of the tech is predicated on the plausibility that the allele variant in question is the most evolutionarily adaptive (maximum fitness or efficiency)
